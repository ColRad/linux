...
static int nxp_nci_i2c_fw_read(struct nxp_nci_i2c_phy *phy, struct sk_buff **skb)
{
	struct i2c_client *client = phy->i2c_dev;
	u16 header;
	size_t frame_len;
...
static int nxp_nci_i2c_nci_read(struct nxp_nci_i2c_phy *phy, struct sk_buff **skb)
{
...
static irqreturn_t nxp_nci_i2c_irq_thread_fn(int irq, void *phy_id)
{
	...
	case NXP_NCI_MODE_FW:
		...
		break;
	case NXP_NCI_MODE_COLD:r = -EREMOTEIO;
		break;
	}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (!phy) {
		nfc_err(&client->dev,
			"Cannot allocate memory for nxp-nci i2c phy\n");
		r = -ENOMEM;
		goto probe_exit;
	}
