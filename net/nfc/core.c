...
int nfc_vendor_rsp(struct nfc_dev *dev, u32 vendor_id, u32 vendor_subcmd,
		   u8 data_len, u8* data)
{
	int rc;

	pr_debug("%s vendor %X subcmd %X\n", dev_name(&dev->dev), vendor_id,
		 vendor_subcmd);

	rc = nfc_genl_vendor_rsp(dev, vendor_id, vendor_subcmd, data_len, data);

	return rc;
}
EXPORT_SYMBOL(nfc_vendor_rsp);
