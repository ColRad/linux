...
static void nci_rf_set_listen_mode_routing_rsp_packet(struct nci_dev *ndev,
						      struct sk_buff *skb)
{
	struct nci_core_set_config_rsp *rsp = (void *) skb->data;
...
void nci_rsp_packet(struct nci_dev *ndev, struct sk_buff *skb)
{
	...
	case NCI_OP_RF_SET_LISTEN_MODE_ROUTING_CMD:
		nci_rf_set_listen_mode_routing_rsp_packet(ndev, skb);
		break;

	default:
		if (ndev->ops->recv_proprietary_rsp_packet)
			rc = ndev->ops->recv_proprietary_rsp_packet(ndev,
								    rsp_opcode,
								    skb);
		if (rc == -ENOSYS)
			pr_err("unknown rsp opcode 0x%x\n", rsp_opcode);
		else
			nci_req_complete(ndev, rc);
		break;
	}
