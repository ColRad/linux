				...
				void *params)
{
	...
	if (rf_tech_and_mode == NCI_NFC_A_PASSIVE_POLL_MODE ||
	    rf_tech_and_mode == NCI_NFC_A_ACTIVE_POLL_MODE) {
		...
		if (target->sensb_res_len > 0) {
		   ...
		   rf_tech_and_mode == NCI_NFC_F_ACTIVE_POLL_MODE) {
			...
			struct nci_rf_intf_activated_ntf *ntf, __u8 *data)
{
	...
	switch (ntf->activation_rf_tech_and_mode) {
	case NCI_NFC_A_ACTIVE_POLL_MODE:
	case NCI_NFC_F_ACTIVE_POLL_MODE:
	case NCI_NFC_A_PASSIVE_POLL_MODE:
	case NCI_NFC_F_PASSIVE_POLL_MODE:
...
exit:
	...
	if (!(ntf.activation_rf_tech_and_mode & NCI_RF_TECH_MODE_LISTEN_MASK)) {
		...
		if (atomic_read(&ndev->state) == NCI_DISCOVERY) {
			/* A single target was found
			 * and activated automatically */
			atomic_set(&ndev->state, NCI_POLL_ACTIVE);
			if (err == NCI_STATUS_OK)
				nci_target_auto_activated(ndev, &ntf);
		} else if (!(atomic_read(&ndev->state) == NCI_IDLE)) {
